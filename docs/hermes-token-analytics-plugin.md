...
- When the sync runs on an hourly cadence, the plugin still emits a zero-valued hourly rollup for idle hours so downstream charts do not go flatline-by-omission
